...
export default observer(() => {
  ...
  const toCopy = async () => {
    ...
    await stores.editor.app.tree
      .export("png", { blob: true, pixelRatio: ratio })
      .then(async (result) => {
        const { data } = result;
        ...
        {stores.option.frameConf.width && (
          <div className="text-xs p-3 mt-4 flex justify-between bg-black/5 rounded-md">
            ...
            <span className="text-gray-700">
              {stores.option.frameConf.width * ratio} x{" "}
              {stores.option.frameConf.height * ratio}
            </span>
          </div>
        )}
      </div>
            ...
            >
              <div className="leading-4 px-2">
                ...
                <div className="text-xs">
                  {ratio}x as {format.toLocaleUpperCase()}
                </div>
              </div>
            </Button>
          </Tooltip>
